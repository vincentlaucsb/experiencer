...
test('loads, dismisses, and resets stable string hint keys', () => {
    const host = createPersistence(JSON.stringify([HintKey.FieldOptions]));
    const store = new HintStore(host.persistence);
    const listener = jest.fn();
    store.subscribe(listener);

    expect(store.isDismissed(HintKey.FieldOptions)).toBe(true);
    store.dismiss(HintKey.FieldOptions);
    expect(listener).not.toHaveBeenCalled();

    store.reset();
    expect(store.isDismissed(HintKey.FieldOptions)).toBe(false);
...
test('synchronizes external storage changes and ignores unknown keys', () => {
    ...
    host.setExternalValue(JSON.stringify(['unknown', HintKey.FieldOptions]));
...
test('persists a dismissal exactly once', () => {
    ...
    expect(host.persistence.write).toHaveBeenCalledWith([HintKey.FieldOptions]);
});
